  ...
  module Steps
    ...
    describe AttachDetachDisk do
      ...
      it "invokes attach_detach_disk" do
        @client.should_receive(:invoke_and_wait).with(:post, @attach_disk_link, kind_of(Hash)) { @attach_disk_link }
        @client.should_receive(:reload).once.ordered.with(@vm) { @vm }
        @client.should_receive(:reload).once.ordered.with(@disk) { @disk }
